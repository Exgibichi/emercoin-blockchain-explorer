...
<table class="table">
	<tr><td><h3>'.lang("BALANCE_BALANCE").'</h3></td><td width="75%"><h3><span class="label label-success">'.TrimTrailingZeroes(number_format(round($balancetotal,7),8)).' NK</span></h3></td></tr>';
foreach ($addressbalances as $address => $value) {
	if ($value>0) {echo '<tr><td><a href="/address/'.$address.'"><button type="button" class="btn btn-link" style="padding:0">'.$address.'</button></a></td><td>'.TrimTrailingZeroes(number_format(round($value,7),8))." NK</td></tr>";}
}
echo '</table>';
